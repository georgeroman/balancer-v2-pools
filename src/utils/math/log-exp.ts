...
// https://github.com/balancer-labs/balancer-core-v2/blob/master/pkg/solidity-utils/contracts/math/LogExpMath.sol

import BigNumber from "../big-number";

const DECIMALS = new BigNumber("1000000000000000000"); // 10^18
const DOUBLE_DECIMALS = DECIMALS.times(DECIMALS); // 10^36

const PRECISION = new BigNumber("100000000000000000000"); // 10^20
const PRECISION_LOG_UNDER_BOUND = DECIMALS.minus("100000000000000000"); // 10^17
const PRECISION_LOG_UPPER_BOUND = DECIMALS.plus("100000000000000000"); // 10^17
 ...
 * @notice Min x is log(0.000000000000000001) = -41.446531673892822312
 * @return eˆx
 */
export function n_exp(x: BigNumber): BigNumber {
  ...
  let t = x;
  s = s.plus(t);
  t = t.times(x).idiv(new BigNumber(2)).idiv(PRECISION);
  s = s.plus(t);
  t = t.times(x).idiv(new BigNumber(3)).idiv(PRECISION);
  s = s.plus(t);
  t = t.times(x).idiv(new BigNumber(4)).idiv(PRECISION);
  s = s.plus(t);
  t = t.times(x).idiv(new BigNumber(5)).idiv(PRECISION);
  s = s.plus(t);
  t = t.times(x).idiv(new BigNumber(6)).idiv(PRECISION);
  s = s.plus(t);
  t = t.times(x).idiv(new BigNumber(7)).idiv(PRECISION);
  s = s.plus(t);
  t = t.times(x).idiv(new BigNumber(8)).idiv(PRECISION);
  s = s.plus(t);
  t = t.times(x).idiv(new BigNumber(9)).idiv(PRECISION);
  s = s.plus(t);
  t = t.times(x).idiv(new BigNumber(10)).idiv(PRECISION);
  s = s.plus(t);
  t = t.times(x).idiv(new BigNumber(11)).idiv(PRECISION);
  s = s.plus(t);
  t = t.times(x).idiv(new BigNumber(12)).idiv(PRECISION);
  s = s.plus(t);

  return ans.times(s).idiv(PRECISION).times(last).idiv(new BigNumber(100));
}
...
export function pow(x: BigNumber, y: BigNumber): BigNumber {
  ...
  if (PRECISION_LOG_UNDER_BOUND.lt(x) && x.lt(PRECISION_LOG_UPPER_BOUND)) {
    ...
    logx_times_y = logbase
      .idiv(DECIMALS)
      .times(y)
      .plus(logbase.mod(DECIMALS).times(y).idiv(DECIMALS));
  } else {
